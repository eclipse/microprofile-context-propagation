...
/**
 * This interface offers various methods for capturing the context of the current thread and applying it to various
 * interfaces that are commonly used with completion stages and executor services. This allows you to contextualize
 * specific actions that need access to the context of the creator/submitter of the stage/task.
 *
 * <p>
 * Example usage:
 * </p>
 ...
 *
 * <p>
 * This interface is intentionally kept compatible with ContextService, with the hope that its methods might one day be
 * contributed to that specification.
 * </p>
 */
public interface ThreadContext {
    ...
    interface Builder {
        /**
         * <p>
         * Builds a new {@link ThreadContext} instance with the configuration that this builder represents as of the
         * point in time when this method is invoked.
         * </p>
         *
         * <p>
         * After {@link #build} is invoked, the builder instance retains its configuration and may be further updated to
         * represent different configurations and build additional <code>ThreadContext</code> instances.
         * </p>
         *
         * <p>
         * All created instances of {@link ThreadContext} are destroyed when the application is stopped. The container
         * automatically shuts down these {@link ThreadContext} instances and raises <code>IllegalStateException</code>
         ...
         *             <ul>
         *             <li>if one or more of the same context types appear in multiple of the following sets:
         ...
         *             <li>if more than one <code>ThreadContextProvider</code> has the same thread context
         ...
         *
         * <p>
         * {@link ThreadContext#ALL_REMAINING} is automatically appended to the set of cleared context if neither the
         * {@link #propagated} set nor the {@link #unchanged} set include {@link ThreadContext#ALL_REMAINING}.
         * </p>
         *
         * <p>
         ...
         *
         * <p>
         * The MicroProfile Config property, <code>mp.context.ThreadContext.cleared</code>, establishes a default that
         ...
         * This context is later re-established on the thread(s) where the action or task executes.
         * </p>
         *
         * <p>
         ...
         *
         * <p>
         * The MicroProfile Config property, <code>mp.context.ThreadContext.propagated</code>, establishes a default
         ...
         *
         * <p>
         * Thread context types which are not otherwise included in this set or in the {@link #unchanged} set are
         * cleared from the thread of execution for the duration of the action or task.
         * </p>
         *
         * @param types
        ...
        /**
         * <p>
         * Defines a set of thread context types that are essentially ignored, in that they are neither captured nor are
         * they propagated or cleared from thread(s) that execute the action or task.
         * </p>
         *
         * <p>
         * This set replaces the <code>unchanged</code> set that was previously specified on the builder instance.
         * </p>
         *
         * <p>
         * Constants for specifying some of the core context types are provided on {@link ThreadContext}. Other thread
         ...
         *
         * <p>
         * The MicroProfile Config property, <code>mp.context.ThreadContext.unchanged</code>, establishes a default that
         * is used if no value is otherwise specified. The value of the MicroProfile Config property can be the empty
         * string or a comma separated list of context type constant values. If a default value is not specified by
         * MicroProfile Config, then the default value is an empty set.
         * </p>
         *
         * <p>
         * The configuration of <code>unchanged</code> context is provided for advanced patterns where it is desirable
         * to leave certain context types on the executing thread.
         * </p>
         *
         * <p>
         ...
         *
         * @param types
         *            types of thread context to leave unchanged on the thread.
         * @return the same builder instance upon which this method is invoked.
         */
     ...
     * <code>cleared</code>, <code>propagated</code>, or <code>unchanged</code>.
     * </p>
     *
     * <p>
     * When using this constant, be aware that bringing in a new context provider or updating levels of an existing
     * context provider might change the set of available thread context types.
     * </p>
     *
     ...
     * with a thread. For Jakarta/Java EE applications, application context includes the thread context class loader as
     * well as the java:comp, java:module, and java:app name spaces of the application. An empty/default application
     * context means that the thread is not associated with any application.
     *
     * @see ManagedExecutor.Builder#cleared
     * @see ManagedExecutor.Builder#propagated
     * @see ThreadContext.Builder
     ...
     * <code>builder.propagated(ThreadContext.NONE)</code> rather than <code>builder.propagated(new String[0])</code> or
     * <code>builder.propagated()</code>, all of which have the same meaning.
     * </p>
     *
     * <p>
     ...
     * with the thread. Implementations are not expected to propagate transaction context across threads. Instead, the
     * concept of transaction context is provided for its cleared context, which means the active transaction on the
     * thread is suspended such that a new transaction can be started if so desired. In most cases, the most desirable
     * behavior will be to leave transaction context defaulted to cleared (suspended), in order to prevent dependent
     * actions and tasks from accidentally enlisting in transactions that are on the threads where they happen to run.
     *
     * @see ManagedExecutor.Builder#cleared
     * @see ManagedExecutor.Builder#propagated
     * @see ThreadContext.Builder
     ...
     *
     * <p>
     * The returned <code>Executor</code> must raise <code>IllegalArgumentException</code> if an already-contextualized
     * <code>Runnable</code> is supplied to its <code>execute</code> method.
     * </p>
     *
     ...
     * Finally, the previous context is restored on the thread, and the result of the <code>Callable</code> is returned
     * to the invoker.
     * </p>
     *
     * @param <R>
     ...
     *             if an already-contextualized <code>Callable</code> is supplied to this method.
     */
    <R> Callable<R> contextualCallable(Callable<R> callable);

    /**
     * <p>
     * Wraps a <code>BiConsumer</code> with context that is captured from the thread that invokes
     ...
     * run the <code>accept</code> method, then the <code>accept</code> method of the provided <code>BiConsumer</code>
     * is invoked. Finally, the previous context is restored on the thread, and control is returned to the invoker.
     * </p>
     *
     * @param <T>
     *            type of first parameter to consumer.
     * @param <U>
     *            type of second parameter to consumer.
     * @param consumer
     *            instance to contextualize.
     * @return contextualized proxy instance that wraps execution of the <code>accept</code> method with context.
     * @throws IllegalArgumentException
     *             if an already-contextualized <code>BiConsumer</code> is supplied to this method.
     */
     ...
     * invoked. Finally, the previous context is restored on the thread, and control is returned to the invoker.
     * </p>
     *
     * @param <T>
     *            type of parameter to consumer.
     * @param consumer
     *            instance to contextualize.
     * @return contextualized proxy instance that wraps execution of the <code>accept</code> method with context.
     * @throws IllegalArgumentException
     *             if an already-contextualized <code>Consumer</code> is supplied to this method.
     */
    <T> Consumer<T> contextualConsumer(Consumer<T> consumer);

    /**
     * <p>
     * Wraps a <code>BiFunction</code> with context that is captured from the thread that invokes
     ...
     * run the <code>apply</code> method, then the <code>apply</code> method of the provided <code>BiFunction</code> is
     * invoked. Finally, the previous context is restored on the thread, and the result of the <code>BiFunction</code>
     * is returned to the invoker.
     * </p>
     *
     * @param <T>
     *            type of first parameter to function.
     * @param <U>
     *            type of second parameter to function.
     * @param <R>
     ...
     * @throws IllegalArgumentException
     *             if an already-contextualized <code>BiFunction</code> is supplied to this method.
     */
     ...
     * invoked. Finally, the previous context is restored on the thread, and the result of the <code>Function</code> is
     ...
     * @return contextualized proxy instance that wraps execution of the <code>apply</code> method with context.
     * @throws IllegalArgumentException
     *             if an already-contextualized <code>Function</code> is supplied to this method.
     */
     ...
     * @throws IllegalArgumentException
     *             if an already-contextualized <code>Runnable</code> is supplied to this method.
     */
     ...
     * invoked. Finally, the previous context is restored on the thread, and the result of the <code>Supplier</code> is
     * returned to the invoker.
     * </p>
     *
     * @param <R>
     ...
     * @throws IllegalArgumentException
     *             if an already-contextualized <code>Supplier</code> is supplied to this method.
     */
     ...
     *
     * <p>
     * The new completable future will use the same default executor as this ThreadContext, which can be a
     * ManagedExecutor if this ThreadContext was obtained by {@link ManagedExecutor#getThreadContext()} or the default
     * executor service if provided by the platform (which can be done via
     ...
     * {@link java.lang.UnsupportedOperationException} for all <code>*Async</code> methods that do not specify an
     * executor. For example, {@link java.util.concurrent.CompletionStage#thenRunAsync(Runnable)
     * thenRunAsync(Runnable)}.
     * </p>
     *
     * <p>
     * When dependent stages are created from the new completable future, thread context is captured and/or cleared as
     * described in the documentation of the {@link ManagedExecutor} class, except that this ThreadContext instance
     * takes the place of the default asynchronous execution facility in supplying the configuration of
     * cleared/propagated context types. This guarantees that the action performed by each stage always runs under the
     * thread context of the code that creates the stage, unless the user explicitly overrides by supplying a
     ...
     *
     * <p>
     * The new completion stage will use the same default executor as this ThreadContext, which can be a ManagedExecutor
     * if this ThreadContext was obtained by {@link ManagedExecutor#getThreadContext()} or the default executor service
     * if provided by the platform (which can be done via
     * {@link ContextManager.Builder#withDefaultExecutorService(ExecutorService)}), or otherwise have no default
     * executor.
     * </p>
     * 
     ...
     *
     * <p>
     * When dependent stages are created from the new completion stage, thread context is captured and/or cleared as
     * described in the documentation of the {@link ManagedExecutor} class, except that this ThreadContext instance
     * takes the place of the default asynchronous execution facility in supplying the configuration of
     * cleared/propagated context types. This guarantees that the action performed by each stage always runs under the
     * thread context of the code that creates the stage, unless the user explicitly overrides by supplying a
