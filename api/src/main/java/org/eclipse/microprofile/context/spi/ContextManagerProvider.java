 ...
 * <code>ContextManager</code> per class loader, which in turn create new instances of
 * {@link org.eclipse.microprofile.context.ManagedExecutor.Builder ManagedExecutor.Builder} and
 * {@link org.eclipse.microprofile.context.ThreadContext.Builder ThreadContext.Builder}.
 * </p>
 *
 * <p>
 ...
 * <code>ContextManagerProvider</code> implementation via the aforementioned {@link ServiceLoader} mechanism. If there
 * are more than one such implementation registered, the {@link #instance()} method will throw an exception as
 * documented
 * </p>
 */
public interface ContextManagerProvider {
    ...
    /**
     * Obtains the <code>ContextManagerProvider</code> instance that has been previously registered, or uses
     * {@link ServiceLoader} to load and register a <code>ContextManagerProvider</code> from the current context class
     * loader.
     *
     * @return the registered <code>ContextManagerProvider</code> instance.
     * @throws IllegalStateException
     *             if there are no registered <code>ContextManagerProvider</code> and we could not discover any via
     *             {@link ServiceLoader}, or if there are more than one {@link ServiceLoader} results.
     */
    ...
    /**
     * Allows the container to register the <code>ContextManagerProvider</code> implementation. At most one
     * implementation can be registered at any given point in time. In order to register a different implementation, the
     * container must first unregister its previous implementation.
     *
     * @param provider
     ...
     */
    public static ContextManagerProviderRegistration register(ContextManagerProvider provider)
            throws IllegalStateException {
     ...
     *             {@link ThreadContextProvider#getThreadContextType type}
    ...
    /**
     * Gets a {@link ContextManager} for the given {@link ClassLoader}. If there is already a {@link ContextManager}
     * registered for the given {@link ClassLoader} or the context manager provider uses a single fixed set of
     * {@link ThreadContextProvider} regardless of the class loader, the existing instance will be returned. If not, one
     * will be created, either by provider-specific mechanisms if {@link ContextManager.Builder} is not supported, or
     * with a {@link ContextManager.Builder} using the specified {@link ClassLoader} (with
     * {@link ContextManager.Builder#forClassLoader(ClassLoader)}) and with
     * {@link ContextManager.Builder#addDiscoveredThreadContextProviders()} called in order to load all
     * {@link ThreadContextProvider} discoverable from the given {@link ClassLoader}. If created, the new
     * {@link ContextManager} will then be registered for the given {@link ClassLoader} with
     * {@link #registerContextManager(ContextManager, ClassLoader)}.
     *
     * @param classloader
     *            the class loader for which to obtain the context manager.
     ...
     *             <code>ThreadContextProvider</code> or is inseparable from the container.
     */
     ...
     * @param classLoader
     *            The {@link ClassLoader} to register it for
     * @throws UnsupportedOperationException
     *             if the <code>ContextManagerProvider</code> always uses the same set of
     *             <code>ThreadContextProvider</code> or is inseparable from the container.
     * @see #getContextManager(ClassLoader)
